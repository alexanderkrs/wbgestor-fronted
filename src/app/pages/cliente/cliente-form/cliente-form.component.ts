...
export class ClienteFormComponent {
  ...
  constructor(
    ...
    private usuarioClientService: ClienteClientService
  ) {
    ...
    if (this.acaoSistema.isAcaoAlterar() || this.acaoSistema.isAcaoVisualizar()) {
      this.cliente = route.snapshot.data.usuario;
      this.telefonesUsuario = this.cliente.telefones;
      this.gruposVinculados = this.cliente.grupos;
      this.dataSourceGrupos.data = this.gruposVinculados;
    }
  }
  ...
  public validarCpf(): void {
    if (this.cliente.cpf === undefined || this.cliente.cpf.length !== 11) {
      delete this.cliente.cpf;
    } else {
      // Verifica se o CPF informado é válido e se está em uso
      this.usuarioClientService.validarCpf(this.cliente.cpf, this.cliente.id).subscribe(() => {}, error => {
        delete this.cliente.cpf;
        this.messageService.addMsgDanger(error);
      });
    }
