...
export class ClienteClientService {
  ...
  public getUsuariosADByFiltro(filtroDTO: FiltroUserKeycloakDTO): Observable<any> {
    return this.http.get(`${environment.urlApi}/cliente/ad/filtro`, {
      params: filtroDTO.toParams()
    });
  }
  ...
  public ativar(id: number): Observable<any> {
    return this.http.put(`${environment.urlApi}/cliente/${id}/ativo`, {});
  }
  ...
  public inativar(id: number): Observable<any> {
    return this.http.put(`${environment.urlApi}/cliente/${id}/inativo`, {});
  }
  ...
  public validarCpf(cpf: string, id: number): Observable<any> {
    ...
    if (id === undefined) {
      observable = this.http.get(`${environment.urlApi}/cliente/cpf/valido/${cpf}`);
    } else {
      observable = this.http.get(`${environment.urlApi}/cliente/${id}/cpf/valido/${cpf}`);
    }
    return observable;
  }
}
